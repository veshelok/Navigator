...
class List<T>::ListIterator {
 public:
  ...
  explicit ListIterator(Node* node) : node_ptr_{node} {}
  ListIterator(const ListIterator& other) : node_ptr_{other.node_ptr_} {}
  ListIterator(ListIterator&& other) : node_ptr_{nullptr} {
  ...
  ~ListIterator() {}
  T& operator*() const { return node_ptr_->value; }
...
class List<T>::ConstListIterator : public List<T>::Iterator {
 public:
  ...
  const T& operator*() const { return this->node_ptr_->value; }
};
...
template <typename T>
void List<T>::Splice(ConstIterator pos,
                     List& other) {  // NOLINT(runtime/references)
  if (this == &other) {
    return;
  }
  if (other.Empty()) {
